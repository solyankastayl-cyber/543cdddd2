/**
 * OVERVIEW PAGE — Market Verdict in One Screen
 * 
 * Structure (8 blocks):
 * A) Top Banner — Market Verdict (Stance, Confidence, Horizon)
 * B) Action Card — What to do
 * C) 3 Reasons — Why this verdict
 * D) 3 Risks — What can break the scenario
 * E) Signal Stack — Key indicators
 * F) Pipeline Summary — Signal transformation flow
 * G) Forecast by Horizon — Table with projections
 * H) Mini Chart — Actual vs Predicted
 * 
 * DESIGN: Light theme, functional colors only (green/orange/red/gray)
 * NO shadows, NO borders, NO dark backgrounds
 */
...
// Lazy load PredictionChart
const PredictionChart = lazy(() => import('../components/charts/PredictionChart'));
...
const getActionHintDescription = (hint, stance) => {
  switch (hint) {
    case 'INCREASE_RISK': 
...
const VerdictBanner = ({ verdict, asset, horizon }) => {
  ...
  return (
    <div className={`p-6 rounded-lg ${getStanceBgColor(verdict.stance)}`}>
  ...
  return (
    <div className="p-5 bg-white border border-gray-100 rounded-lg">
      ...
      <p className="text-sm text-gray-600">
        {getActionHintDescription(verdict.actionHint, verdict.stance)}
      </p>
    </div>
  );
};
...
const MetaFooter = ({ meta, latencyMs }) => {
  ...
  return (
    <div className="flex items-center justify-between text-xs text-gray-400 pt-4 border-t border-gray-100">
      <div className="flex items-center gap-4">
        <span>Version: {meta.systemVersion}</span>
        <span>Data: {meta.dataMode}</span>
...
export default function OverviewPage() {
  ...
  // Horizon selector
  const horizonOptions = [30, 90, 180, 365];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">
              One-screen verdict with actionable insights
            </p>
          </div>
        ...
        {data && (
          <div className="space-y-4">
            ...
            {/* B) Action Card + C) Reasons + D) Risks */}
            ...
            {/* H) Prediction Chart — History Layer */}
            <Suspense fallback={<div className="p-4 text-gray-500">Loading chart...</div>}>
              <PredictionChart 
